...
├── components/
│   ├── Nav.jsx               # Navigation with Communities dropdown
...
### Navigation
- Added "Blog" link to nav (was missing)
- Added "Communities" dropdown in nav linking to all 6 town pages
- All nav items are styled consistently
...
- Fields: address, zipCode, name, email, phone
...
### Facebook Pixel
- Pixel ID placeholder: `YOUR_PIXEL_ID_HERE` in App.jsx
- **IMPORTANT: Replace with actual pixel ID before deployment**
- Tracks:
  - PageView (all pages)
...
2. **Add Facebook Pixel ID**
   - Find: `YOUR_PIXEL_ID_HERE` in `/src/App.jsx`
   - Replace with your actual Facebook Pixel ID
   - There are 2 instances (one in script, one in noscript)
...
2. TownPage component will automatically handle the route
3. Update sitemap.xml
4. Update nav Communities dropdown (automatic via towns loop)
